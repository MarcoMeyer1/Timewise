...
class TimesheetDetails : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val timesheetId = intent.getIntExtra("timesheetId", -1)
        val timesheet = TimesheetManager.timesheets.find { it.id == timesheetId }
    ...
    private fun displayTimesheetDetails(timesheet: Timesheet) {
        ...
        val pieEntries = listOf(
            PieEntry(userHours, userHours.toString()),
            PieEntry(minLeft.toFloat(), minLeft.toString()),
            PieEntry(maxLeft.toFloat(), maxLeft.toString())
        )
